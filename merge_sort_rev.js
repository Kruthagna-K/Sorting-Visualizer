function Merge_rev()
{
    ...
    merge_partition_rev(0,array_size-1);

    enable_buttons();
}

function merge_sort_rev(start,mid,end)
{
    var p=start,q=mid+1;
...
function merge_partition_rev(start,end)
{
    if(start < end)
    {
        ...
        merge_partition_rev(start,mid);
        merge_partition_rev(mid+1,end);

        merge_sort_rev(start,mid,end);
    }
}
